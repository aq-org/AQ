...
void HandleImport(Interpreter& interpreter, Ast::Import* statement) {
  ...
  // Store the module interpreter pointer for advanced use cases
  ...
  // First, copy the main class (.!__start)
  ...
  // BUT: Skip classes that are themselves imports (they start with "~import~")
  for (auto& imported_class_pair : imported_interpreter->classes) {
    ...
    // Skip classes that are imports from the imported module (avoid nested imports)
    if (imported_class_name.find("~import~") == 0) {
      LOGGING_INFO("Skipping nested import class: '" + imported_class_name + "'");
      continue;
    }
...
void HandleClassInHandlingVariable(Interpreter& interpreter,
  ...
  if (dot_pos != std::string::npos) {
    ...
    if (alias_it != interpreter.import_alias_to_class_name.end()) {
      ...
      // The imported class name would be like "~import~test"
      // We need to construct the full class name as "~import~test.CLASS_NAME"
